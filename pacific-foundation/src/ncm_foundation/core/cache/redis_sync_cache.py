"""
Redis-based cache implementation using synchronous redis library for ncm-foundation.
...
class RedisSyncCache:
    """Redis-based cache implementation using synchronous redis library."""
    ...
    async def set(
        ...
        try:
            ...
            await asyncio.to_thread(self.redis.setex, full_key, ttl, data)
            self._sets += 1
    ...
    async def delete(self, key: str) -> bool:
        ...
        try:
            full_key = self._get_key(key)
            result = await asyncio.to_thread(self.redis.delete, full_key)
            self._deletes += 1
    ...
    async def exists(self, key: str) -> bool:
        ...
        try:
            full_key = self._get_key(key)
            result = await asyncio.to_thread(self.redis.exists, full_key)
            return bool(result)
    ...
    async def clear_pattern(self, pattern: str) -> int:
        ...
        try:
            ...
            keys = await asyncio.to_thread(self.redis.keys, full_pattern)
    ...
    async def get_many(self, keys: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            data_list = await asyncio.to_thread(self.redis.mget, full_keys)
    ...
    async def close(self) -> None:
        ...
        try:
            ...
            if self.pool:
                await asyncio.to_thread(self.pool.disconnect)
    ...
    async def extend_ttl(self, key: str, ttl: int) -> bool:
        ...
        try:
            full_key = self._get_key(key)
            result = await asyncio.to_thread(self.redis.expire, full_key, ttl)
